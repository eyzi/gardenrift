...
pub fn graphics(app_name: [:0]const u8, allocator: std.mem.Allocator) !void {
    ...
    // state.objects.window = try visual.vulkan.instance2.window.create(.{
    //     .app_name = app_name,
    //     .width = 400,
    //     .height = 300,
    // });
    // defer visual.vulkan.instance2.window.destroy(.{
    //     .window = state.objects.window,
    // });

    // // visual.vulkan.window.create(state.configs.app_name, state.configs.initial_window_width, state.configs.initial_window_height);
