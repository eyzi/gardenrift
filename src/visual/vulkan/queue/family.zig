...
pub fn create_info(params: struct {
    ...
    var queue_create_infos = try std.ArrayList(glfwc.VkDeviceQueueCreateInfo).initCapacity(params.allocator, unique_indices.items.len);
...
pub fn get_properties(params: struct {
    physical_device: glfwc.VkPhysicalDevice,
    allocator: std.mem.Allocator,
}) ![]glfwc.VkQueueFamilyProperties {
    var n_properties: u32 = undefined;
    glfwc.vkGetPhysicalDeviceQueueFamilyProperties(params.physical_device, &n_properties, null);
    if (n_properties == 0) {
        return error.DeviceQueueFamilyNone;
    }

    var property_list = try params.allocator.alloc(glfwc.VkQueueFamilyProperties, n_properties);
    glfwc.vkGetPhysicalDeviceQueueFamilyProperties(params.physical_device, &n_properties, property_list.ptr);
...
pub fn is_present_supported(params: struct {
    ...
    if (glfwc.vkGetPhysicalDeviceSurfaceSupportKHR(params.physical_device, params.queue_family_index, params.surface, &supported) != glfwc.VK_SUCCESS) {
        return false;
    }
